...
class CultivationService:
    ...
    def end_cultivation(self, player: Player) -> CultivationResult:
        ...
        # 功法和其他已装备物品的修炼倍率此前只参与显示，
        # 现在统一在出关结算时读取并真正应用。
        equipment_exp_bonus = 0.0
        if self.equipment_service is not None:
            equipment_bonuses = self.equipment_service.get_equipment_bonuses(
                player.user_id
            )
            equipment_exp_bonus = max(
                -0.99,
                float(equipment_bonuses.exp_multiplier)
            )

        # 计算获得的修为
        gained_exp = self._calculate_cultivation_exp(
            player,
            effective_minutes,
            technique_bonus=equipment_exp_bonus
        )
